# theGoodintro — Codex Onboarding

This file is loaded by Codex on every session. Keep it tight. Update it
...
> `apps/platform/AGENTS.md` is mandatory reading and its rules are non-negotiable.**
...
| Codex Design (Codex.ai/design) | All visual iteration, with live preview |
...
| Design iteration | Done in Codex Design at Codex.ai/design with live preview, **not** in Codex | Existing rule: design there, port here. |
...
**Policy is Issy's, not Codex's.** Codex writes code that implements decisions;
