...
export async function POST(req: NextRequest) {
  ...
  const record = {
    ...
    outreachVolume: str(body.outreachVolume, 60),
    outreachToday: str(body.outreachToday, 120),
    needToSee: str(body.needToSee, 3000),
    meetingsPerYear: str(body.meetingsPerYear, 60),
    meetingLength: str(body.meetingLength, 60),
    ...
    priorities: str(body.priorities, 2000),
    relevantCategories: str(body.relevantCategories, 600),
    wouldRefer: str(body.wouldRefer, 30),
    impactProfile: str(body.impactProfile, 60),
  ...
  console.log(`[apply] LEAD ${record.email} (${record.company}) utm=${record.utmSource}`);
  return NextResponse.json({ ok: true });
}
