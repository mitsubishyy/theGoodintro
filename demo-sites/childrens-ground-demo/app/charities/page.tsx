...
// Highlighted charity first, then 8 dimmed neighbours -> a tidy 3x3 grid.
const RSPCA = CHARITIES.find((c) => c.slug === HIGHLIGHT_SLUG);
const OTHERS = CHARITIES.filter((c) => c.slug !== HIGHLIGHT_SLUG).slice(0, 8);
const NINE = RSPCA ? [RSPCA, ...OTHERS] : CHARITIES.slice(0, 9);

function CardInner({
  c,
}: {
  c: (typeof CHARITIES)[number];
}) {
  return (
    <>
      ...
      <div className="text-[17px] font-semibold tracking-tight">{c.name}</div>
      <div className="mt-1.5 text-[13px]" style={{ color: "var(--cream-9)" }}>
        {c.tagline}
      </div>
