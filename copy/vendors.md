# For vendors (/vendors)

Page title: For vendors. TheBigIntro.
Meta description: A qualified introduction to a vetted, hard-to-reach senior audience. Every meeting you book sends $1,000 to the leader's chosen charity. Australia first, invite only.

## Hero

Eyebrow: For vendors
H1: Reach leaders **honestly**.
Lede: A genuinely qualified introduction to a vetted, hard-to-reach senior audience. Earned by being relevant, not by buying a list and sending more cold email.
CTA: Book a call
Trust: You fund the giving. Every meeting you book sends $1,000 to the leader's chosen charity.

## What you get

Eyebrow: What you get
H2: A room you cannot cold-email into.

- A vetted senior audience — Leaders who opted in and set their own priorities. Not a scraped list, and not someone screening on their behalf.
- A qualified introduction — You state the specific initiative. It reaches the leader only when it genuinely fits what they asked to hear about.
- Goodwill that lasts — Every meeting sends $1,000 to the leader's chosen charity. You are remembered as the introduction that did some good.

## Who this is for

Eyebrow: Who this is for
H2: Built for vendors who mean it.

- Socially minded — You are glad that the value you create here also sends a substantial gift to a cause the leader chooses.
- Genuine intent — You have a specific initiative or problem to discuss and can state it plainly. No vague discovery, no laboured demos.
- Meet on their terms — One focused conversation, led by the leader's priorities. No hard selling. The relationship is the point.

## What is expected

Eyebrow: What is expected
H2: State your reason up front.
Lede: Before a request reaches a leader you describe the specific initiative or challenge that makes the conversation relevant. That one requirement is what keeps quality high on both sides, and it is why senior leaders agree to be here at all.

## Pricing

Eyebrow: What it costs, in full
H2: Transparent pricing.

Big number: $1,000

- To the leader's chosen charity, per meeting — the full meeting gift
- Platform admin fee — billed to you, separately and clearly named
- What the leader pays — nothing, ever

Lede: The donation is never reduced to cover our costs. The admin fee is its own named line, so you always know exactly what funds the charity and what funds the platform.

Give flow:
- After the meeting — the donation is sent to the leader's chosen registered charity
- You get confirmation — in writing, every time
- The admin fee — is billed to you, never blended with the gift

## Founding vendors

Eyebrow: Founding vendors
H2: Help shape how this works.
We are starting small on purpose. Founding vendors help shape how this works, alongside the first leaders who join.
CTA: Book a call

## Closing

Eyebrow: The invitation
H2: Be the introduction worth taking.
Reach senior leaders by being relevant, and fund a real gift while you do it. Every meeting you book sends $1,000 to the leader's chosen charity.
CTA: Book a call
Sub: One short call to start. You fund the giving.
